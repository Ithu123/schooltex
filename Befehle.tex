...
\setkeys{image}{width=5cm, caption=, source=, position=, overhang=0.5 cm, line =  }
  ...
  \ifdefempty{\image@caption}{
    ...
    \ifdefempty{\image@source}{
      \renewcommand{\mycaption}{\protect\caption{\image@caption}}
    }{
      \renewcommand{\mycaption}{\protect\caption{\image@caption\protect\footnotemark}}
      \footnotetext[\theimagesourcecounter]{Quelle: \url{\image@source}}
...
\makeatletter
\define@key{multiimage}{caption}{\def\mycaption{#1}} % Define a key for the caption
...
\newcommand{\multiimage}[2][]{ % Optional arguments are provided as a key-value list
  \setkeys{multiimage}{caption=, label=, rescale=1, #1} % Set default values and apply user-provided values
  ...
  \readlist\mylist{#2} % Read and store the elements of the list

  % Calculate the width of each subfigure based on the number of elements and rescale factor
  \newlength{\subfigurewidth}
  \setlength{\subfigurewidth}{\dimexpr(0.8\linewidth/\mylistlen)\relax}
