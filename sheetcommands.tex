...
\newbool{materialpresent}
\boolfalse{materialpresent}

\ProvideDocumentCommand{\material}{O{} m}{ % Optional arguments are provided as a key-value list, m=  mandatory argument = Text
    \booltrue{materialpresent} % Tell Aufgabe, that material is present
}



% Define a global temporary variable
\newcommand{\tempmaterial}{}

% Define a command to store material content
\newcommand{\storematerial}[1]{\gdef\tempmaterial{#1}}

% Define a command to process and print stored material
\newcommand{\printmaterial}{%
    \StrSubstitute{\tempmaterial}{\material{\tempmaterial}}{}[\processedmaterial]% Remove \material{...} from \tempmaterial
    \processedmaterial% Print the processed content
}

% Define a custom environment
\newenvironment{removeMaterial}{%
    \let\oldmaterial\material% Save the old \material definition
    \renewcommand{\material}[1]{\storematerial{##1}}% Redefine \material to store content
}{%
    \let\material\oldmaterial% Restore the old \material definition
}
...
\ProvideDocumentEnvironment{Aufgabe}{O{0} O{1} +b }{%
% Replace \material{...} with nothing, while storing \material contents
    ...
    \paragraph{\margin_symbol\Aufg\, \theAufgabe} \setlength{\AufgP}{#1 pt}  \setcounter{TA}{1} \refstepcounter{Aufgabe}  
    }{
\ifdim\AufgP=0pt
    \relax
\else
...
\fi
    ...
    \boolfalse{materialpresent} % reset material
    }{}

\ProvideDocumentCommand{\teilaufgabe}{ O{0} m}{%
    \setlength{\pointspace}{0.4cm}
    ...
    \relax
    \else
    \marginnote{(#1 P.)}[-1\baselineskip] % Punktanzahl für die Teilaufgabe am Rand
...
% \ProvideDocumentCommand{\solutionpart}{m}{%
%     \setlength{\parskip}{0.3em}
%     \begin{addmargin}[1em]{0em}
...
% Ab hier steht die funktionierende Lösung bevor Chat darin herumgepfuscht hat
% \makeatletter
% \ProvideDocumentCommand{\printsols}{}{ 
